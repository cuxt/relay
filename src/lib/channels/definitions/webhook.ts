...
export const webhookDefinition: ChannelDefinition<WebhookConfig> = {
  type: 'webhook',
  label: '自定义 Webhook',
  color: '#6366f1',
  ...
  sendMessage: async ({ message, config }: SendContext<WebhookConfig>): Promise<SendResult> => {
    if (!config.webhook) {
    ...
    try {
      ...
      const res = await fetch(config.webhook, init)
